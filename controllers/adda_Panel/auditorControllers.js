...
exports.auditorUserSignup=async(req,res)=>{
    try {
        ...
        const userExist = await auditorUserSchema.findOne({ userEmail: user.userEmail });
        if (userExist) {
        ...
        const nameExist = await auditorUserSchema.findOne({ userName: user.userName });
        if (nameExist) {
          return res.status(403).json({
            status: "Failed",
...
exports.auditorHome= async (req, res) => {
  ...
  exports.auditorAprovedScore = async (req, res) => {
    try {
      ...
      await adgeSchema.findByIdAndUpdate(id, { score: score,status:status }, { new: true });
      await Score.save();
